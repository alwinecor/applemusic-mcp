...
def library(
    ...
    For action='add', `track` accepts one or more song names or catalog song IDs
    in a single call. Pass multiple tracks as a comma- or newline-separated string
    (e.g. track='Hey Jude, Let It Be'), or a JSON array encoded as a string
    (e.g. track='["Hey Jude", "Let It Be"]'). Prefer one batch call when adding
    multiple songs. Use newlines or JSON if names contain commas.
    """
    action = action.lower().strip().replace("-", "_")
