...
| Reorder playlist tracks | `playlist(action='reorder', playlist='p.…', from_position=5, to_position=2)` | Positions are 1-based in the current API order. Alternatively pass a full permutation such as `order='3,1,2'`; every position must appear exactly once. `dry_run=True` previews. Requires web sign-in and a web-editable playlist on every OS, independent of playback mode. Preserves the playlist ID and every track occurrence; verifies by default. Read the playlist again after an uncertain result before retrying. |
